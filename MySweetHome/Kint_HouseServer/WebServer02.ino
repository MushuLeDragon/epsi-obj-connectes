...
void setup() {
  ...
  */
  ...
  // Initialisation du port SD
  if (!SD.begin(4)) {
    Serial.println("Initialisation du port SD impossible.");
    return;
  }
  Serial.println("Initialisation du port SD...");
}
...
void sendResponse(EthernetClient client) {
  ...
  //displayClientBody(client);

  // Affichage des fichiers au client
  displayClientFile(client);
...
/**
 * Affiche la réponse du serveur dans le corps du client
 */
void displayClientBody(EthernetClient client) {
  if (strcmp(url_parts[0], "hello") == 0) {
    client.println(F("Hello world!!"));
  } else if (strcmp(url_parts[0], "bye") == 0) {
    client.println(F("Bye bye le bouseux!"));
  } else {
    client.println(F("Index"));
  }
}
 ...
 */
void displayClientFile(EthernetClient client) {
  ...
  if (fichier) {
    ...
    client.print(F("Erreur: impossible d'ouvrir le fichier demande"));
  }
}

int8_t getMethod(char c) {
  int8_t readBytes = 0, method = 0;
  while (method < 4) {
    if (c == methods[method][readBytes++]) {
      if (readBytes == strlen(methods[0])) {
        return method;
      } else {
        readBytes = 0;
 ...
 */
char *concat_string(char *s1, char const *s2) {
    char *tmp = s1;
