...
static const uint8_t URL_PART_SIZE = 20;
/**
...
// Choix de la methode 1 = GET, 2 = POST, 3 = PUT, 4 = DELETE
uint8_t method = 0;
...
bool digestURL(char c) {
  static int8_t algoPart = 0, readBytes = 0, urlSize = 0, postOrPut = 0;
  if (algoPart == 0) {
    // On vérifie que le caractère lue correspond à un caractère d'une méthode
    int8_t actualIndex = readBytes++;
    if(postOrPut == 0){
    if (c == GET[actualIndex]) {
    ...
    } else if(c == POST[actualIndex]){
      postOrPut = 1;
    } else if(c == DELETE[actualIndex]){
      if (readBytes == strlen(DELETE)) {
        // On vient de lire 'DELETE', on passe à la suite
        algoPart++;
        readBytes = 0;
        method = 4;
      }
      ...
      readBytes = 0;
       ...
       if (c == POST[actualIndex]) {
        // On vient de lire 'POST', on passe à la suite
        algoPart++;
        readBytes = 0;
        method = 2;
      }else if(c == PUT[actualIndex]){
        // On vient de lire 'PUT', on passe à la suite
        algoPart++;
        readBytes = 0;
        method = 3;
      }
...
void displayContent(EthernetClient client) {
   for (uint8_t i = 0; i < nbParam; i++) {
      if(String(param_names[i]) == "name"){
         client.println(F("Hello "));
         client.print(param_values[i]);
